...
/**
 * This is the LateralZeroPowerAccelerationTuner autonomous follower OpMode. This runs the tests.robot
 * to the right until a specified velocity is achieved. Then, the tests.robot cuts power to the motors, setting
 * them to zero power. The deceleration, or negative acceleration, is then measured until the tests.robot
 * stops. The accelerations across the entire time the tests.robot is slowing down is then averaged and
 * that number is then printed. This is used to determine how the tests.robot will decelerate in the
 ...
 * You can adjust the max velocity the tests.robot will hit on FTC Dashboard: 192/168/43/1:8080/dash
 *
...
public class LateralZeroPowerAccelerationTuner extends OpMode {
    ...
    public void init() {
        ...
        telemetryA.addLine("The tests.robot will run to the right until it reaches " + VELOCITY + " inches per second.");
     ...
     * game pad 1 will stop the OpMode. When the tests.robot hits the specified velocity, the tests.robot will
